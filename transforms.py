...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        if 'disp' in sample.keys():
            disp = np.transpose(sample['disp'], (2, 0, 1))  # [1, H, W]
...
class RandomCrop(object):
    ...
    def __call__(self, sample):
        ...
        if self.img_height > ori_height or self.img_width > ori_width:
            ...
            if 'disp' in sample.keys():
                sample['disp'] = self.crop_disp(sample['disp'])
                sample['mask'] = self.crop_disp(sample['mask'])
            if 'pseudo_disp' in sample.keys():
    ...
    def crop_disp(self, img):
        return img[self.offset_y *2:(self.offset_y + self.img_height)*2,
               self.offset_x*2:(self.offset_x + self.img_width)*2]
...
class RandomVerticalFlip(object):
    ...
    def __call__(self, sample):
        if np.random.random() < 0.5:
            ...
            sample['mask'] = np.copy(np.flipud(sample['mask']))
