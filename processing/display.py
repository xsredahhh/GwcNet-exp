...
def display_color_disparity_depth(epoch, step, writer, colors, pred_disparity, pred_depth, gt_depth, mask, phase='Training', is_return_img=False, color_reverse=True):
    pred_depth = pred_depth[:, 2, :, :].unsqueeze(1).data.cpu()
    gt_depth = gt_depth[:, 2, :, :].unsqueeze(1).data.cpu()
    mask = (mask > 0.6)[:, 2, :, :].unsqueeze(1).data.cpu()
    ...
    pred_depth_display = cv2.applyColorMap(
        np.uint8(255 * np.moveaxis(pred_depth_display.numpy(), source=[0, 1, 2], destination=[2, 0, 1])),
        cv2.COLORMAP_JET)
    gt_depth_display = cv2.applyColorMap(np.uint8(255 * np.moveaxis(gt_depth_display.numpy(), source=[0, 1, 2], destination=[2, 0, 1])), cv2.COLORMAP_JET)
